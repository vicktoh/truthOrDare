...
export const LevelScreen: FC<Props> = ({ navigation, route: { params } }) => {
  const { option } = params;
  const levels: Level[] = [
    {
      title: "Normal",
  ...
  return (
    <View style={styles.container}>
        <Text style = {[styles.title]}>{`Choose Level of your ${option} questions`}</Text>
      {levels.map((level, i) => (
        <LevelCard
         key = {`level-${i}`}
          {...level}
          onClick={() => {
            navigation.push("Question", { option, level: level.title, background: level.background, color: level.textColor || Colors.light.secondaryBrownDark, icon: level.icon });
          }}
        />
      ))}
      <TouchableOpacity
